...
export function Builded() {
  ...
  const cards = [
    ...
    },
    {
      ...
      description: "PC Desktop Gaming MSI MAG Codex 5 13NUC5-1649IT – Intel Core i5-13400F, 16GB(8GB*2), GeForce RTX 4060, SSD 1TB, Windows 11 Home",
        ...
        <div className="builded-carousel">
          <div className="card-container">
            {cards.map((card, index) => (
              <div 
                key={index} 
                className={`card-content ${currentCard === index ? 'active' : 'hidden'}`}
              >
                <div className="card-background"></div>
                <div className="card-image" style={{backgroundImage: `url(${card.backgroundImage})`}}></div>
                <div className="card-info">
                  <h2 className="card-title">{card.title}</h2>
                  <p className="card-description">{card.description}</p>
                  <hr className="card-divider" />
                  <div className="card-price-container">
                  <span className="card-original-price">{card.originalPrice}</span>
                  <span className="card-discounted-price">{card.discountedPrice}</span>
                  <Button text="Buy Now" />
                </div>
                </div>
              </div>
